...
function mix_switch_page(mix_page, push_to_history = true) {
  try {
    ...
    if (!el.getAttribute("mix-y")) {
      el.setAttribute("mix-y", "0");
    }
    ...
    if (push_to_history) {
      history.pushState(
        { mix_page: el.getAttribute("mix-url") },
        "title",
        el.getAttribute("mix-url")
      );
    }
  } catch (error) {
...
function mixhtml() {
  try {
    ...
    if (el.hasAttribute("mix-patch")) {
      url = el.getAttribute("mix-patch");
      method = "PATCH";
    }
    ...
    if (url == "") {
      console.error("mixhtml() - url missing");
      return;
    }
    if (el.hasAttribute("mix-page")) {
      url += url.includes("?") ? "&mix-page=yes" : "?mix-page=yes";
    }
  ...
  try {
    ...
    t.querySelectorAll("*").forEach((element) => {
      if (element.hasAttribute("mix-url")) {
        ...
        if (push_to_history) {
          history.pushState(
            { mix_page: element.getAttribute("mix-url") },
            "title",
            element.getAttribute("mix-url")
          );
        }
      ...
      if (element.hasAttribute("mix-before")) {
        document
          .querySelectorAll(element.getAttribute("mix-before"))
          .forEach((el) => {
            console.log(element);
      ...
      if (element.hasAttribute("mix-top")) {
        document
          .querySelectorAll(element.getAttribute("mix-top"))
          .forEach((el) => {
      ...
      if (element.hasAttribute("mix-bottom")) {
        document
          .querySelectorAll(element.getAttribute("mix-bottom"))
          .forEach((el) => {
      ...
      if (element.hasAttribute("mix-after")) {
        document
          .querySelectorAll(element.getAttribute("mix-after"))
          .forEach((el) => {
      ...
      if (element.hasAttribute("mix-update")) {
        document
          .querySelectorAll(element.getAttribute("mix-update"))
          .forEach((el) => {
      ...
      if (element.hasAttribute("mix-replace")) {
        document
          .querySelectorAll(element.getAttribute("mix-replace"))
          .forEach((el) => {
            element.removeAttribute("mix-replace");
            if (element.hasAttribute("mix-url")) {
              console.log(
                "mix-replace not possible on single page app element"
              );
            } else {
      ...
      if (element.hasAttribute("mix-remove")) {
        document
          .querySelectorAll(element.getAttribute("mix-remove"))
          .forEach((el) => {
            el.remove();
          });
      }
...
function mix_convert() {
  document
    .querySelectorAll(
      "[mix-url], [mix-get], [mix-post], [mix-patch], [mix-put], [mix-delete], [mix-ttl]"
    )
    .forEach((el) => {
      try {
        ...
        if (el.hasAttribute("mix-change")) {
          el_event = "onchange";
        }
        if (el.hasAttribute("mix-input")) {
          el_event = "oninput";
        }
        if (el.hasAttribute("mix-url") && !el.getAttribute("mix-on")) {
          el.setAttribute("mix-on", "yes");
        }
        if (el.hasAttribute("mix-url") && !el.hasAttribute("mix-ttl")) {
          el.setAttribute("mix-ttl", "0");
        }
        if (el.hasAttribute("mix-ttl")) {
          if (el.hasAttribute("mix-fade-out")) {
            const ttl = el.hasAttribute("mix-ttl")
              ? el.getAttribute("mix-ttl")
              : 2000;
            el.setAttribute(
              "style",
              `animation: mix-fade-out ${ttl}ms forwards;`
            );
          }
